...
# The disc icons are generated by fluxstyle (fluxstyle.icon_png: a white glyph on
# a disc that carries the logo's own gradient). Site file name → fluxstyle glyph.
ICONS = {
    "icon-thermal-capture": "thermal",
    "icon-acoustic-amplification": "acoustic",
    "icon-solid-state-conversion": "conversion",
    "icon-ai-optimization": "ai",
    "icon-seamless-integration": "integration",
    "icon-heat-modulation": "heatmod",
    "icon-economic-compulsion": "economic",
    "icon-commercial-validation": "validation",
    "icon-regulation-mandates": "mandates",
}
ICON_PX = 400


def sync_icon_assets(force=False):
    """Write the disc icons into assets/images/ from fluxstyle. Rasterising needs
    headless Chromium, so a plain build writes only the icons that are missing;
    `python build.py --icons` rewrites all of them (after a fluxstyle change)."""
    dest_dir = os.path.join("assets", "images")
    written = 0
    for filename, glyph in ICONS.items():
        dest = os.path.join(dest_dir, f"{filename}.png")
        if force or not os.path.isfile(dest):
            fluxstyle.icon_png(glyph, dest, size=ICON_PX)
            written += 1
    print(f"Icons: {written} written from fluxstyle, {len(ICONS) - written} kept.")
...
def build(check_only=False):
    ...
    sync_logo_assets()
    sync_icon_assets(force="--icons" in sys.argv)
